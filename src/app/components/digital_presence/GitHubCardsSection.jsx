...
const CARD_BASE =
  'bg-surface border border-outline rounded-[0.75em] p-[0.75em] shadow-md ' +
...
function SkeletonCard({ className = '', scale = '1rem' }) {
  return (
    <div className={`${CARD_BASE} ${className} animate-pulse`} style={{ fontSize: scale }}>
      <div className="h-[0.75em] w-1/3 rounded-[0.2em] bg-outline/20 mb-[0.75em]" />
      <div className="h-[0.5em] w-full rounded-[0.15em] bg-outline/10 mb-[0.5em]" />
      <div className="h-[0.5em] w-4/5 rounded-[0.15em] bg-outline/10 mb-[0.5em]" />
      <div className="h-[0.5em] w-2/3 rounded-[0.15em] bg-outline/10" />
    </div>
  );
}
...
function ErrorCard({ title, className = '', scale = '1rem' }) {
  return (
    <div className={`${CARD_BASE} ${className} flex items-center justify-center`} style={{ fontSize: scale }}>
      <div className="text-center">
        <span className="material-symbols-outlined text-on-surface-variant text-[1.5em]">error_outline</span>
        <p className="text-[0.625em] text-on-surface-variant mt-[0.25em]">{title}</p>
        <p className="text-[0.5625em] text-on-surface-variant opacity-60">Add GITHUB_TOKEN to .env.local</p>
      </div>
    </div>
  );
}
  ...
  useEffect(() => {
    const fetchAll = async () => {
      ...
      if (productive.status === 'fulfilled' && !productive.value.error) {
        setProductiveTimeData(productive.value);
      } else {
  ...
  return (
    <div className="w-full max-w-full h-full flex flex-col items-center gap-[0.5em]">
      ...
      <div className="flex items-center justify-center gap-[0.5em] mb-[0.25em] w-full">
        <svg height="1.125em" viewBox="0 0 16 16" className="text-on-surface-variant opacity-60 flex-shrink-0" fill="currentColor">
          <path d="M8 0c4.42 0 8 3.58 8 8a8.013 8.013 0 0 1-5.45 7.59c-.4.08-.55-.17-.55-.38 0-.27.01-1.13.01-2.2 0-.75-.25-1.23-.54-1.48 1.78-.2 3.65-.88 3.65-3.95 0-.88-.31-1.59-.82-2.15.08-.2.36-1.02-.08-2.12 0 0-.67-.22-2.2.82-.64-.18-1.32-.27-2-.27-.68 0-1.36.09-2 .27-1.53-1.03-2.2-.82-2.2-.82-.44 1.1-.16 1.92-.08 2.12-.51.56-.82 1.28-.82 2.15 0 3.06 1.86 3.75 3.64 3.95-.23.2-.44.55-.51 1.07-.46.21-1.61.55-2.33-.66-.15-.24-.6-.83-1.23-.82-.67.01-.27.38.01.53.34.19.73.9.82 1.13.16.45.68 1.31 2.69.94 0 .67.01 1.3.01 1.49 0 .21-.15.45-.55.38A7.995 7.995 0 0 1 0 8c0-4.42 3.58-8 8-8Z" />
        </svg>
        <span className="text-[16px] font-semibold text-on-surface-variant tracking-wider uppercase">My GitHub Stats</span>
      </div>
